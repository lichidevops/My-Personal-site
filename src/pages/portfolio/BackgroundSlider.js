...
export default function BackgroundSlider() {
  ...
  return (
    ...
    <div id='backgroundSlider-page'>
        <div className='backgroundSlider-container'>
        {
            slides.map((slide,index)=>{
                return (
                    <div key={index} className={`background-slide ${slide.state}`} style={{backgroundImage:`url(${slide.url})`}}>
                    </div>
                )
            }
            )
        }
        <button className='backgroundSlider-arrow bgLeft-arrow' id='backgroundSlider-left'>
            <FontAwesomeIcon icon={faArrowLeft}/>
        </button>

        <button className='backgroundSlider-arrow bgRight-arrow' id='backgroundSlider-right'>
